...
function createPopup(currentFeature) {
  ...
  const popup = new mapboxgl.Popup({ closeOnClick: true })
    ...
    .setHTML(
      "<div class='card-header'><img src='/assets/weed.svg' width='20px' />&nbsp;<h3>" +
        currentFeature.properties[config.popupTitle] +
        "</h3></div><div class='card-data'><p id='description' class='txt-s py12 none block-ml'>" +
        currentFeature.properties[config.popupAddress] +
        "</p></div><div class='card-footer'><div class='map-icon-container'><a href='tel:" +
        currentFeature.properties[config.popupPhone] +
        "'><img src='/assets/phone-call.svg' alt='Call Dispensary' class='map-icons' /></a></div><div class='map-icon-container'><a href='" +
        currentFeature.properties[config.popupURL] +
        "' alt='Dispensary Location'><img src='/assets/external-link.svg' alt='Dispensary Website' class='map-icons' /></a></div></div>"
    )
    .addTo(map);
}
...
function buildCheckbox(title, listItems) {
  ...
  filterTitle.classList.add("center", "flex-parent", "txt-bold");
